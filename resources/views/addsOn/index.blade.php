        ...
        <div class="content-wrapper">
            <div class="content-header row">
                <div class="content-header-left col-md-6 col-12 mb-2 breadcrumb-new">
                    <h3 class="content-header-title mb-0 d-inline-block">{{ __('main.Adds-on-list') }} {{__('main.for_restaurant')}}
                        @if (App::getLocale() == 'en')
                            {{$restaurant->name_en}}
                        @else
                            {{$restaurant->name_ar}}
                        ...
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                ...
                                <li class="breadcrumb-item active"><a
                                        href="{{ route('addsOn.index',$restaurant->id) }}">{{ __('main.Adds-on-list') }}</a>
                                </li>
                            </ol>
                        </div>
                    </div>
                        ...
                        <div class="card">
                            <div class="card-head">
                                <div class="card-header">
                                    <h3 class="form-section"><i class="la la-reorder font-large-1"></i>
                                        {{ __('main.Adds_on_categories') }}
                                    </h3>
                                    ...
                                    <div class="heading-elements">
                                        ...
                                        <button type="button" class="btn btn-warning btn-sm" data-toggle="modal"
                                            ...
                                            {{ __('main.add-adds_on-category') }}</button>
                                    </div>
                                    {{-- start of add modal --}}
                                    <!-- Modal -->
                                            ...
                                            <div class="modal-content">
                                                <div class="modal-header bg-warning white">
                                                    <h4 class="modal-title white" id="myModalLabel12">
                                                        {{ __('main.add-new-adds_on-category') }}</h4>
                                                    <button type="button" class="close" data-dismiss="modal"
                                                            aria-label="Close">
                                                        <span aria-hidden="true">&times;</span>
                                                    </button>
                                                </div>
                                                <form action="{{route('addsOn.store',$restaurant->id)}}" method="POST" name="form0" onsubmit="return validateForm(event)">
                                                    @csrf
                                                    <div class="modal-body">
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <fieldset class="form-group">
                                                                <label for="type">{{ __('main.choose-type') }} </label>
                                                                <select class=" form-control"  id="type" name="type">
                                                                    <option value="" disabled>{{ __('main.choose-type') }}</option>
                                                                    <option value="1">{{ __('main.single') }} </option>
                                                                    <option value="2">{{ __('main.multiple') }} </option>
                                                                </select>
                                                                @error('type')
                                                                <small  class="form-text text-danger">{{$message}}</small>
                                                                @enderror
                                    ...
                                    <div class="table-responsive">
                                        <table id=""
                                            ...
                                            <tr>
                                                ...
                                                <th class="text-center">{{ __('main.category-name-en') }}</th>
                                                <th class="text-center">{{ __('main.type') }}</th>
                                                <th class="text-center">{{ __('main.Adds_on') }}</th>
                                                <th class="text-center">{{ __('main.actions') }}</th>
                                            </tr>
                                            </thead>
                                            <tbody>
                                            @foreach($adds_on_categories as $category)
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        <a href="{{route('addsOn_element.index',$category->id)}}"  > <button type="button" class="btn btn-warning btn-sm" ><i class="white"></i>
                                                                {{ __('main.Adds_on') }}</button></a>
                                                    </td>
                                                    <td class="text-center">
                                                       <span class="dropdown">
                                                           ...
                                                           <span aria-labelledby="SearchDrop2"
                                                               ...
                                                               <a class="dropdown-item primary" data-toggle="modal"
                                                                  data-target="#edit_form_{{$category->id}}" category_id ={{$category->id}} id="category_edit">
                                                                   ...
                                                                   {{ __('main.edit') }}</a>
                                                               <a href="{{route('addsOn.delete',$category->id)}}" class="dropdown-item danger">
                                                ...
                                                <div class="modal fade text-left" id="edit_form_{{$category->id}}" tabindex="-1"
                                                    ...
                                                    <div class="modal-dialog" role="document">
                                                        <div class="modal-content">
                                                            ...
                                                            <form action="{{ route('addsOn.update',$category->id) }}" method="POST" name="form{{$category->id}}" onsubmit="return validateForm(event)">
                                                                @csrf
                                                                <div class="modal-body">
                                                                    <h5><i class="la la-arrow-right"></i>
                                                                    ...
                                                                    <div class="form-group">
                                                                        ...
                                                                        <fieldset class="form-group">
                                                                            ...
                                                                            <select class=" form-control"  id="type" name="type">
                                                                                <option value="" disabled>{{ __('main.choose-type') }}</option>
                                                                                <option value="1" @if($category->type == 1) selected @endif>{{ __('main.single') }} </option>
                                                                                <option value="2" @if($category->type == 2) selected @endif>{{ __('main.multiple') }} </option>
                                                                            </select>
                                                                            @error('type')
                                                                            <small  class="form-text text-danger">{{$message}}</small>
                                                                            @enderror
...
@endsection
@section('search js')
    @include('includes.addsCategoryValidation')
    {{--    create--}}
    @if (Session::has('create_msg_Adds_on_category'))
        @if (App::getLocale() == 'ar')
            <script>
                toastr.success('{{ Session::get('create_msg_Adds_on_category') }}', '{{ Session::get('success_title') }}', {
                    "showMethod": "fadeIn",
                    "hideMethod": "fadeOut",
                    timeOut: 3000,
    ...
    @endif
    {{--    update--}}
    @if (Session::has('update_msg_Adds_on_category'))
        @if (App::getLocale() == 'ar')
            <script>
                toastr.success('{{ Session::get('update_msg_Adds_on_category') }}', '{{ Session::get('success_title') }}', {
                    "showMethod": "fadeIn",
                    "hideMethod": "fadeOut",
                    timeOut: 3000,
                    positionClass: 'toast-top-left',
    ...
    @endif
    {{--    not found--}}
    @if (Session::has('not_found_msg_Adds_on_category'))
        @if (App::getLocale() == 'ar')
            <script>
                toastr.error('{{ Session::get('not_found_msg_Adds_on_category') }}', '{{ Session::get('error_title') }}', {
                    "showMethod": "fadeIn",
                    "hideMethod": "fadeOut",
                    timeOut: 3000,
                    positionClass: 'toast-top-left',
    ...
    @endif
    {{--    Can't Delete--}}
    @if (Session::has('cant_delete_msg_Adds_on_category'))
        @if (App::getLocale() == 'ar')
            <script>
                toastr.error('{{ Session::get('cant_delete_msg_Adds_on_category') }}', '{{ Session::get('error_title') }}', {
                    "showMethod": "fadeIn",
                    "hideMethod": "fadeOut",
                    timeOut: 3000,
                    positionClass: 'toast-top-left',
    ...
    @endif
    {{--    delete--}}
    @if (Session::has('delete_msg_Adds_on_category'))
        @if (App::getLocale() == 'ar')
            <script>
                toastr.success('{{ Session::get('delete_msg_Adds_on_category') }}', '{{ Session::get('success_title') }}', {
                    "showMethod": "fadeIn",
                    "hideMethod": "fadeOut",
                    timeOut: 3000,
                    positionClass: 'toast-top-left',
